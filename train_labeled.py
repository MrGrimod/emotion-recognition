...
def main():
    x_train, y_train, x_test, y_test, input_shape = load_data('dataset/ck_dataset_labeld.pickle', True)
    ...
    tbCallBack = keras.callbacks.TensorBoard(log_dir='./dataset/labeld_training_tb', histogram_freq=0, write_graph=True, write_images=True)
    model.fit(x_train, y_train, validation_data=(x_test, y_test), batch_size=30, epochs=200, callbacks=[tbCallBack])
    model.save_weights('dataset/train_labeld_weights.h5')
